...
def run():
    for i in range(100):
        ...
        z = (pts * z_axis).sum(axis=-1)
        x = (pts * x_axis).sum(axis=-1)
        y = (pts * y_axis).sum(axis=-1)
        x = (x / -z / np.tan(fovy / 2) / aspect) * 0.5 + 0.5
        y = (y / -z / np.tan(fovy / 2)) * 0.5 + 0.5
        z = utils3d.numpy.project_depth(-z, near, far)
        expected = np.stack([x, y, z], axis=-1)
        
        actual, _ = utils3d.numpy.project_gl(points, None,
