...
| `utils3d.solve_poses_sequential`<br>Given trajectories of points over time, sequentially solve for the poses (transformations from canonical to each frame) of each body at each frame. | [`utils3d.np.solve_poses_sequential(trajectories, weights, accum, min_valid_size, mode, lam, niter)`](utils3d/numpy/pose.py#L240) | - | 
